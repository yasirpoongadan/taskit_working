...
export class CompanyDashboardComponent implements OnInit {
  ...
  // ---------------------------------------------------Project Vs Task Hours----------------------------------------------
  getHoursforResourceGraph() {
    // ---------------------------------Start-------------------------------------------
    ...
    this.companyService.getAllProject().subscribe(data => {
      ...
      data.forEach(element => {
        if (element.status == "Planned" || element.status == "Completed" || element.status == "In Progress") {
          this.projects3.push(element);
          this.years.push(element.project_name)
        }
      });
      if (this.projects3.length == 0) {
        //no data in charts
      } else {
        this.exists = true;
        this.companyService.getHoursforResourceGraph(this.projects3).subscribe(info => {
          // console.log(info)s
          this.drawGraph(this.offenseNames, this.years, info);
      });
    };
    });
    // -----------------------------------End------------------------------------------
  ...
  getAllProjects1() {
    this.companyService.getAllProject().subscribe(data => {
      ...
      data.forEach(element => {
        if (element.status == "Planned" || element.status == "Completed" || element.status == "In Progress") {
          this.projects1.push(element);
        }
      });
      if (this.projects1.length == 0) {
      } else {
        this.project_id = this.projects1[0].id;
        this.getData1(this.project_id);
      }
    });
  }

  changeProject1(pro_id) {
    this.getData1(pro_id);
  }
 ...
 getAllProjects2() {
  this.companyService.getAllProject().subscribe(data => {
    ...
    data.forEach(element => {
      if (element.status == "Planned" || element.status == "Completed" || element.status == "In Progress") {
        this.projects2.push(element);
      }
    });
    if (this.projects2.length == 0) {
      this.users = [];
      this.proExists = false;
      //no data in charts
    } else {
      this.proExists = true;
      console.log("yasir" +this.projects2[0].id)
      this.project_id = this.projects2[0].id;
      this.getTeamMembers(this.projects2[0].id);
    }
  });
}

changeProject2(pro_id) {
...
getTeamMembers(projId) {
  ...
  this.companyService.getUsers(projId).subscribe(data => {
    ...
    if (this.users.length == 0) {
      ...
      this.getData2(projId, this.users);
    }
  });
}

getData2(projId, users) {
  // ---------------------------------Start-------------------------------------------
  // Function      : getTasksforResourceGraph
  // Params        : pro_id, users
  // Returns       : 
  ...
  this.companyService.getTasksforResourceGraph(projId, users).subscribe(data => {
    ...
    this.drawGraph2(this.offenseNames, this.years, this.offensesByYear);
  });
}


drawGraph2(offenseNames, years, offensesByYear) {
